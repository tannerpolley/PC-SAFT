...
_component_params = {
    "H2O": {
        ...
        # "vol_mol_liq_comp": ConstantProperties,
    ...
    "CO2": {
        ...
        "parameter_data": {
            ...
            # Copy ideal gas properties for liquid
            # "cp_mol_liq_comp_coeff": {
            #     "A": 24.99735,
            #     "B": 55.18696,
            #     "C": -33.69137,
    ...
    "PZ": {
        "type": Solvent,
        "cp_mol_liq_comp": LinearCpThermoPure,
        #"diffus_phase_comp": {"Liq": DiffusCO2},
        "enth_mol_liq_comp": LinearCpThermoPure,

        # "vol_mol_liq_comp": VolMolCO2,
        "dens_mol_liq_comp": constant_density,
        "pressure_sat_comp": PressureSatAntoine,
        "relative_permittivity_liq_comp": relative_permittivity_Akula,
        # "henry_component": {
        #     "Liq": {
        #         "method": HenryCO2ChenEtAl,
        ...
        # },
        "parameter_data": {
            "mw": (0.08515, pyunits.kg / pyunits.mol),
            "pressure_sat_comp_coeff":{
                "A": (5.9832*log(10), pyunits.dimensionless),
                "B": (1215.77*log(10), pyunits.K),
                "C": (-113.56, pyunits.K),
                "D": (1.0, pyunits.kPa)               
            },
            "relative_permittivity_liq_comp_coeff": {
            ...
            "cp_mol_liq_comp_coeff": {
                # From Chen et al. (2010), pp 54-59
                "A": 46.69,
                "B": 0.4315,
            },
            # "henry_coeff": {
            #     # Calculated from multiplying Psat from the Antoine equation in Hartono et al. (2021)
            #     # by the infinite dilution activity coefficient from Hillard (2008) then fitting
            #     # to this form using Excel. Mixing data from different sources won't give a great
            #     # result, but this is only intended to be a starting point
            #     # "1": (-6756.19, pyunits.K),
            #     # "2": (0.0, pyunits.dimensionless), # Fixed at this value
            #     # "3": (-0.0137, 1/pyunits.K),
            #     # "4": (22.467, pyunits.dimensionless),
            #     # Warm started from a previous solution
            #     "1": (-8874.803863166795, pyunits.K),
            #     "2": (0.0, pyunits.dimensionless), # Fixed at this value
            #     "3": (-0.008886202520681959, 1/pyunits.K),
            #     "4": (35.246446853869806, pyunits.dimensionless),
            # },
            # "diffus_phase_comp_coeff": {
            #     "1": 2.35e-6,
            #     "2": 2.9837e-8,
...
_inherent_rxn_dict = {
    ...
    ("H2O", "PZ"): {
        "PZ_protonation": {
            ...
            "parameter_data": {
                "k_eq_coeff": {
                    # Hetzer, Robinson, and Bates (1968)
                    # converted from molality to mole fraction
                    # with symmetric activity coeff for PZ
                    # Data collected at temp. range of 0 to 50 C
                    "1": (1.19697970123526, pyunits.dimensionless),
                    "2": (8265.33475405506, pyunits.K),
                    "3": (0.0, pyunits.dimensionless),
                    "4": (0.00320832427446046, 1/pyunits.K),
                }
            },
        },
    },
    ...
    ("H2O", "CO2", "PZ"): {
        "PZ_carbamate_formation": {
            ...
            "parameter_data": {
                "k_eq_coeff": {
                    # Values from Cullinane and Rochelle (2005)
                    # Need to correct first term for lack of reference temperature
                    "1": (-10.15-21980.0/298.15-44.42*log(298.15), pyunits.dimensionless),
                    "2": (21980.0, pyunits.K),
                    "3": (44.42, pyunits.dimensionless),
                    "4": (0.0, 1/pyunits.K)
                }
            },
        },
        "PZ_carbamate_protonation": {
            "stoichiometry": {
                ("Liq", "PZCOO^-"): -1,
                ("Liq", "H3O^+"): -1,
                ("Liq", "PZH^+COO^-"): 1,
            ...
            "parameter_data": {
                "k_eq_coeff": {
                    # Values from Cullinane and Rochelle (2005)
                    # Need to correct first term for lack of reference temperature
                    # Note equilibrium constant in paper is given for reverse rxn
                    "1": (25.91-5700.0/298.15, pyunits.dimensionless),
                    "2": (5700.0, pyunits.K),
                    "3": (0.0, pyunits.dimensionless),
                    "4": (0.0, 1/pyunits.K)
                }
        ...
        "PZ_bicarbamate_formation": {
            ...
            "parameter_data": {
                "k_eq_coeff": {
                    # Values from Cullinane and Rochelle (2005)
                    # Need to correct first term for lack of reference temperature
                    "1": (-13.26-1990.0/298.15, pyunits.dimensionless),
                    "2": (1990.0, pyunits.K),
                    "3": (0.0, pyunits.dimensionless),
                    "4": (0.0, 1/pyunits.K)
                }
...
_parameter_data = {
    "Liq_alpha": {
        ...
        ("H2O", "PZ"): 0.5247436546796379,
        ("PZ", "H2O"): 0.5247436546796379,
        # ("H2O", "CO2"): 0.2,
        # ("MEA", "CO2"): 0.2,
    ...
    "Liq_tau_A": {
        # Default value of 0
        ("PZ", "H2O"): -2.0796983582966977,
        ("H2O", "PZ"): 8.731049067307989,
    ...
    "Liq_tau_B": {
        # Default value of 0
        ("PZ", "H2O"): 488.1405544091012,
        ("H2O", "PZ"): -3473.1665498462694,
...
def get_prop_dict(components=None, excluded_rxns=None, rxn_combinations=None):
    ...
    assert comp_set.issubset({"H2O", "AMP", "CO2", "PZ"})
    configuration = {
